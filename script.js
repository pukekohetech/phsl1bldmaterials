...
function submitWork() {
  ...
  hdr.style.cssText = `
    background:#6e1818;color:#fff;padding:10px 14px;border-radius:6px;
    margin-bottom:12px;font-family:sans-serif;
  `;
  hdr.innerHTML = `<strong style="font-size:1.1em;">${APP_TITLE}</strong><br>
                   <span style="font-size:0.9em;">${APP_SUBTITLE}</span>`;
  ansDiv.appendChild(hdr);

  // Summary
  const sum = document.createElement("div");
  sum.style.cssText = "margin-bottom:14px;line-height:1.5;font-size:0.95em;";
  sum.innerHTML = `
    <strong>Assessment:</strong> ${currentAssessment.title} – ${currentAssessment.subtitle}<br>
    <strong>Student:</strong> ${name}${displayUsername ? ` (${displayUsername}${autoTag})` : ""} – ID: ${id}<br>
    <strong>Teacher:</strong> ${teacherName}${teacherEmail ? ` &lt;${teacherEmail}&gt;` : ""}<br>
    <strong>Submitted:</strong> ${submittedAt}<br>
    <strong style="color:#c0392b;">Score: ${total}/${totalPoints} (${pct}%)</strong>
  `;
  ansDiv.appendChild(sum);

  // Separator
  const sep = document.createElement("hr");
  sep.style.cssText = "border:none;border-top:2px solid #ddd;margin:12px 0;";
  ansDiv.appendChild(sep);
  ...
  results.forEach(r => {
    ...
    block.innerHTML = `
      ...
      <strong>Answer:</strong> <em>${r.answer}</em>${hintHtml}
  ...
  foot.style.cssText = "text-align:center;color:#777;font-size:0.85em;margin-top:16px;";
  foot.textContent = "Generated by Pukekohe High School Technology Dept";
  ansDiv.appendChild(foot);
